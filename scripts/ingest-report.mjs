 ...
 *
 * Parses the report's fields (reference, owner, CNIC, values, type-specific
 * details) and upserts them into public.valuation_reports (keyed on reference).
 */
...
const m1 = (re, t) => { const m = t.match(re); return m ? m[1] : null; };

function parseDate(s) {
  if (!s) return null;
  const d = new Date(s.replace(/(\d+)(st|nd|rd|th)/, "$1"));
  if (isNaN(d)) return null;
  // use local components to avoid a UTC off-by-one
...
function parse(text) {
  ...
  let owner_name = ownerLine, owner_guardian = null;
  const og = (ownerLine || "").match(/^(.+?),\s*(?:wife|son|daughter|husband)\s+of\s+(.+)$/i);
  if (og) { owner_name = clean(og[1]); owner_guardian = clean(og[2]); }
  ...
  const common = {
    ...
    market_value_cad: num(m1(/Canadian Dollars\s+CAD\s*([\d,]+(?:\.\d+)?)/i, t)),
    exchange_rate: num(m1(/1\s*CAD\s*=\s*([\d.,]+)\s*PKR/i, t)),
  };
  ...
  if (valuation_type === "gold") {
    ...
    details = {
      purity_karat: num(m1(/Gold Jewellery\s*\((\d+)\s*Karat\)/i, t)) || num(m1(/assessed at\s+(\d+)\s*karat/i, t)),
